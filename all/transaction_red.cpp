...
private:
    ...
    // Function to reduce transactions based on frequent itemsets
    void reduceTransactions(set<vector<int>>& frequentItemsets) {
        vector<vector<int>> reducedTransactions;

        for (const auto& transaction : transactions) {
            for (const auto& itemset : frequentItemsets) {
                if (includes(transaction.begin(), transaction.end(), itemset.begin(), itemset.end())) {
                    reducedTransactions.push_back(transaction);
                    break; // Only need to keep this transaction once
                }
            }
        }
...
public:
    TransactionReductionApriori(const vector<vector<int>>& trans, int minSup)
    ...
    void run() {
        ...
        while (!candidates.empty()) {
            ...
            // Reduce the transactions based on the found frequent itemsets
            reduceTransactions(frequentItemsets);
...
int main() {
    ...
    TransactionReductionApriori apriori(transactions, minSupport);
    apriori.run();
