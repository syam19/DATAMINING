...
// Function to calculate split information (entropy of the feature itself)
double split_information(const vector<DataPoint> &data, int feature_idx) {
    map<string, int> feature_value_count;
    for (const DataPoint &dp : data) {
        feature_value_count[dp.features[feature_idx]]++;
    }
    double split_info = 0.0;
    int total_size = data.size();
    for (const auto &[value, count] : feature_value_count) {
        double probability = (double)count / total_size;
...
double gain_ratio(const vector<DataPoint> &data, int feature_idx, const vector<string> &unique_labels) {
    ...
    double information_gain = total_entropy - subset_entropy;

    // Calculate split information
    double split_info = split_information(data, feature_idx);
...
// Function to find the best feature to split on using gain ratio
int best_split_feature(const vector<DataPoint> &data, const vector<string> &unique_labels) {
    ...
    double best_gain_ratio = -1;
    int best_feature_idx = -1;
    for (int i = 0; i < num_features; i++) {
        double gain_ratio_value = gain_ratio(data, i, unique_labels);
